...
object MoviesRecommendation {
  ...
  def moviesSimilaritiesTable(inputRDD: RDD[String]): RDD[((Int, Int), (Double, Int))] = {
    ...
    val sortedMoviesSimilarities: RDD[((Int, Int), (Double, Int))] = moviesSimilarities.filter(x => relevantSimilarities(x, minimumSimilarity, minimumPairs)) // UTR - Unary Transformations Replacement
  ...
  def topNMoviesRecommendation(sortedMoviesSimilarities: RDD[((Int, Int), (Double, Int))], n: Int, minimumSimilarity: Double, minimumPairs: Int): RDD[String] = {
    ...
    val relevantMoviesSimilarities: RDD[((Int, Int), (Double, Int))] = sortedMoviesSimilarities.filter(x => relevantSimilarities(x, minimumSimilarity, minimumPairs))
    ...
    val topNRecommendedMoviesByMovie: RDD[String] = sortedRecommendedMoviesList.map(x => makeTopNRecommendedMoviesCSV(x,n))
