...
object MoviesRatingsAverage {
  ...
  def mutantAggregation(x: (Double, Double), y: (Double, Double)): (Double, Double) = {
    val mutated = aggregateDoubleTuple(x, x)
    mutated
  }
  ...
  def moviesRatingsAverage(inputRDD: RDD[String], movieNames: Broadcast[Map[Int, String]]) = {
    ...
    val ratingsSum: RDD[(Int, (Double, Double))] = ratings.reduceByKey(mutantAggregation) // IPOAF
    ...
    val resultsCVS: RDD[String] = ratingsAverageSorted.map(x => mapRatingsAverageToCSV(x, movieNames))
