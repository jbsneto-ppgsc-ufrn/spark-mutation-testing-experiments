...
object MoviesRatingsAverage {
  ...
  def mutantMap(r: String): Option[(Int, (Double, Double))] = {
    val original = parseRatings(r).get
    val mutated = (Int.MinValue, original._2)
    Some(mutated)
  }
  
  def moviesRatingsAverage(inputRDD: RDD[String], movieNames: Broadcast[Map[Int, String]]) = {

    val ratings: RDD[(Int, (Double, Double))] = inputRDD.flatMap(mutantMap) // MTR
