...
object NasaApacheWebLogsAnalysis {
  ...
  def sameHostProblem(firstLogs: RDD[String], secondLogs: RDD[String]): RDD[String] = {
    ...
    val cleanedHostIntersection: RDD[String] = intersection.filter(line => isNotHeader(line)) // UTS - Unary Transformations Swap
  ...
  def unionLogsProblem(firstLogs: RDD[String], secondLogs: RDD[String]): RDD[String] = {
    ...
    val cleanLogLines: RDD[String] = uniqueLogLines.filter(host => isNotHeaderHost(host)) // UTS - Unary Transformations Swap
    cleanLogLines
  }
