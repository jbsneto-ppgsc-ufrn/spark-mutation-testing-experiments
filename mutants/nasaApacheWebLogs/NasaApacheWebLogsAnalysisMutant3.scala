...
object NasaApacheWebLogsAnalysis {
  ...
  def sameHostProblem(firstLogs: RDD[String], secondLogs: RDD[String]): RDD[String] = {
    ...
    val secondHosts: RDD[String] = secondLogs.filter(host => isNotHeaderHost(host)) // UTS - Unary Transformations Swap
    val intersection: RDD[String] = firstHosts.intersection(secondHosts)
    val cleanedHostIntersection: RDD[String] = intersection.map(parseLogs) // UTS - Unary Transformations Swap
