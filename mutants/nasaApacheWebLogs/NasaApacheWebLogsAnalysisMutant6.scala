...
object NasaApacheWebLogsAnalysis {
  ...
  def sameHostProblem(firstLogs: RDD[String], secondLogs: RDD[String]): RDD[String] = {
    ...
    val cleanedHostIntersection: RDD[String] = intersection.distinct() // UTS - Unary Transformations Swap
  ...
  def unionLogsProblem(firstLogs: RDD[String], secondLogs: RDD[String]): RDD[String] = {
    ...
    val uniqueLogLines: RDD[String] = aggregatedLogLines.filter(host => isNotHeaderHost(host)) // UTS - Unary Transformations Swap
