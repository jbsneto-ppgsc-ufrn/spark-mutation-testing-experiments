...
object NasaApacheWebLogsAnalysis {
  ...
  def sameHostProblem(firstLogs: RDD[String], secondLogs: RDD[String]): RDD[String] = {
    val firstHosts: RDD[String] = firstLogs.map(parseLogs) // UTS - Unary Transformations Swap
    val secondHosts: RDD[String] = secondLogs.map(parseLogs) // UTS - Unary Transformations Swap
