...
object NasaApacheWebLogsAnalysis {
  ...
  def unionLogsProblem(firstLogs: RDD[String], secondLogs: RDD[String]): RDD[String] = {
    ...
    val uniqueLogLines: RDD[String] = aggregatedLogLines.filter(line => isNotHeader(line)) // UTS - Unary Transformations Swap
    val cleanLogLines: RDD[String] = uniqueLogLines.distinct() // UTS - Unary Transformations Swap
    cleanLogLines
  }
