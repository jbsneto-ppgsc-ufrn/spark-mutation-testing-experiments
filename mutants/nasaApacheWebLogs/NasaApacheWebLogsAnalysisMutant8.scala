...
object NasaApacheWebLogsAnalysis {
  ...
  def sameHostProblem(firstLogs: RDD[String], secondLogs: RDD[String]): RDD[String] = {
    ...
    val secondHosts: RDD[String] = secondLogs.filter(line => isNotHeader(line)) // UTS - Unary Transformations Swap
  ...
  def unionLogsProblem(firstLogs: RDD[String], secondLogs: RDD[String]): RDD[String] = {
    ...
    val cleanLogLines: RDD[String] = uniqueLogLines.map(parseLogs) // UTS - Unary Transformations Swap
    cleanLogLines
  }
