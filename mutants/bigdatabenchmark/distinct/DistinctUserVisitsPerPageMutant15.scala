...
object DistinctUserVisitsPerPage {
  ...
  def mutantAggregation(a: Set[String], b: Set[String]) = {
    val aggregation = setUnion(a, a)
    aggregation
  }
  ...
  def distinctUserVisitsPerPage(input: RDD[String]) = {
    ...
    val distinctSites: RDD[(String, Set[String])] = mapedUserAccess.reduceByKey(mutantAggregation) // IPOAF - Invertion of the Parameters Order of Aggregation Function
    distinctSites
  }
