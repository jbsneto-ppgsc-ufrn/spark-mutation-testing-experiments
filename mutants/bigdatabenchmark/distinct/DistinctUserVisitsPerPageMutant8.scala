...
object DistinctUserVisitsPerPage {
  ...
  def distinctUserVisitsPerPage(input: RDD[String]) = {
    ...
    val distinctSites: RDD[(String, Set[String])] = mapedUserAccess.reduceByKey(mutantAggregation) // ATR - Aggregation Transformation Replacement
    distinctSites
  }
