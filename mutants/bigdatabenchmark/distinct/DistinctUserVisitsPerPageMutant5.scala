...
object DistinctUserVisitsPerPage {
  ...
  def mutantMap(u: (String, String)) = {
    val original = mapSourceIPToSet(u)
    (original._1, Set(original._2.head))
  }
  ...
  def distinctUserVisitsPerPage(input: RDD[String]) = {
    ...
    val mapedUserAccess: RDD[(String, Set[String])] = userAccesses.map(mutantMap) // MTR - Mapping Transformation Replacement
