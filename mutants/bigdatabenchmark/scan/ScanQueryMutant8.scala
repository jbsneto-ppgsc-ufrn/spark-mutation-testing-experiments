...
object ScanQuery {
  ...
  def mutantMap(r: Ranking) = {
    val original = mapRankingToTuple(r)
    (original._1, Int.MaxValue)
  }
  ...
  def scan(input: RDD[String]): RDD[(String, Int)] = {
    ...
    val results: RDD[(String, Int)] = filteredRankings.map(mutantMap) // MTR - Mapping Transformation Replacement
    results
  }
