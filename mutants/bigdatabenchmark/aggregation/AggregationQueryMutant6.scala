...
object AggregationQuery {
  ...
  def mutantMap(u: UserVisit) = {
    val original = mapUserVisitToTuple(u)
    (original._1, Float.MaxValue)
  }

  def aggregation(input: RDD[String]) = {
    val userVisits: RDD[UserVisit] = input.map(parseUserVisits)
    val userVisitsTuples: RDD[(String, Float)] = userVisits.map(mutantMap) // MTR - Mapping Transformation Replacement
    val results: RDD[(String, Float)] = userVisitsTuples.reduceByKey((x, y) => x + y)
    results
  }
