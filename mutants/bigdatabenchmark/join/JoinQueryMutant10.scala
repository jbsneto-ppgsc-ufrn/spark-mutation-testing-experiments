...
object JoinQuery {
  ...
  def join(rankingsLines: RDD[String], userVisitsLines: RDD[String]): RDD[(String, Float, Int)] = {
    ...
    val subqueryJoin: RDD[(String, (Ranking, UserVisit))] = subqueryR.rightOuterJoin(subqueryUV).map(x => (x._1, (x._2._1.getOrElse(null), x._2._2))) // JTR - Join Transformation Replacement
